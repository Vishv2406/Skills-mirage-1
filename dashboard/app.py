...
def render_tab_a(df, selected_cities, selected_sectors, days):
    ...
    with col1:
        ...
        fig = px.bar(
            ...
            color="Jobs", 
            color_continuous_scale=["#334155", "#60a5fa", "#c084fc"],
    ...
    top_cities = filtered["city"].value_counts().head(12).index.tolist()
    top_sectors = filtered["sector"].value_counts().head(8).index.tolist()
    ...
    if not heatmap_data.empty:
        ...
        fig3 = go.Figure(data=go.Heatmap(
            ...
            x=sector_labels,
            y=city_labels,
            colorscale=[[0, "#1e293b"], [0.3, "#334155"], [0.6, "#60a5fa"], [1, "#c084fc"]],
            hoverongaps=False,
            hoverinfo='skip',
            colorbar=dict(
    ...
    if sector_data:
        ...
        fig4 = px.bar(
            ...
            color="AI Mentions",
            color_continuous_scale=["#334155", "#8b5cf6", "#f472b6"],
...
def render_tab_b(df, courses_df, selected_cities, selected_sectors, days):
    ...
    with col1:
        ...
        # Wrap skill names
        top20["Skill_Display"] = top20["Skill"].apply(lambda x: '<br>'.join([x[i:i+15] for i in range(0, len(str(x)), 15)]))
        
        fig = px.bar(
            top20, x="Demand", y="Skill_Display", orientation="h",
            color="Demand",
            color_continuous_scale=["#1a2a4a", "#3b82f6", "#93c5fd"],
        )
        fig.update_layout(
            height=550,
            showlegend=False,
    ...
    # Build gap analysis
    gap_data = []
    for skill, demand in skill_counts.most_common(25):
        ...
        gap_data.append({
            ...
            "color": "#22c55e" if is_trained else "#ef4444",
        })
    ...
    # Legend manually
    fig3.add_trace(go.Bar(x=[None], y=[None], marker_color="#22c55e", name="✅ Training exists", showlegend=True))
    fig3.add_trace(go.Bar(x=[None], y=[None], marker_color="#ef4444", name="❌ Training gap", showlegend=True))
    ...
    city_skill_data = []
    top_cities = filtered["city"].value_counts().head(10).index.tolist()
    for city in top_cities:
        city_jobs = filtered[filtered["city"] == city]
        sc = compute_skill_trends(city_jobs)
        top3 = [s for s, _ in sc.most_common(3)]
        city_skill_data.append({
            "City": city,
            "Jobs": len(city_jobs),
            "Top Skills": " · ".join(top3) if top3 else "N/A",
            "#1 Skill": top3[0] if top3 else "N/A",
        })
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Top Skills": st.column_config.TextColumn("Top 3 Skills in Demand"),
...
def render_tab_c(df, vuln_df):
    ...
    with col1:
        ...
        if not vuln_df.empty:
            ...
            top_cats = vuln_df.groupby("job_category")["score"].mean().nlargest(12).index
            top_cities = vuln_df.groupby("city")["score"].mean().nlargest(10).index
            pivot = pivot.loc[
    ...
    with col2:
        ...
        top_risk = vuln_df.sort_values("score", ascending=False).head(15)
